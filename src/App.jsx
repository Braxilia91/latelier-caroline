...
function AppInner() {
  ...
  const handleSaveSettings = async ({ name, apiKey, openAiKey, leaVoice, syncToken, editorFont, editorTheme, editorWidth }) => {
    ...
    if (syncToken !== undefined) {
      await db.setSyncToken(syncToken)
    }
    if (editorFont !== undefined) {
      await db.setEditorFont(editorFont)
    }
    if (editorTheme !== undefined) {
      await db.setEditorTheme(editorTheme)
    }
    if (editorWidth !== undefined) {
      await db.setEditorWidth(editorWidth)
    }
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      <Header
        ...
        onDictate={() => setModal('dictation')}
        onPlan={() => setModal('plan')}
        onExport={() => setModal('export')}
        onSettings={() => setModal('settings')}
        onInspir={() => setModal('inspir')}
        onVocab={() => setModal('vocab')}
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        ...
        />
        <WritingArea
          chapter={db.currentChapter}
          updateChapter={db.updateChapter}
          recordSession={db.recordSession}
          editorFont={db.editorFont}
          editorTheme={db.editorTheme}
          editorWidth={db.editorWidth}
        />
        <CoachPanel
        ...
        {modal === 'settings' && (
          <SettingsModal
            state={{
              ...
              openAiKey: db.openAiKey,
              leaVoice: db.leaVoice,
              syncToken: db.syncToken,
              syncStatus: db.syncStatus,
              syncMessage: db.syncMessage,
            ...
            onReset={db.resetAllData}
          />
        )}
        {modal === 'inspir' && (
          <InspirationModal
            onClose={() => setModal(null)}
            onSendToCoach={coach.sendMessage}
            hasKey={!!db.apiKey}
          />
        )}
        {modal === 'export' && (
      ...
      {/* ── Indicateur offline (caché sur mobile pour éviter overlap bouton envoyer) ── */}
      {!isMobile && (
        <div
          style={{
            position: 'fixed',
